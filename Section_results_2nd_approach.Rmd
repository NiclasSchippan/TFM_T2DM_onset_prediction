...
```{r}
input_cox_trend <- read_rds("/home/niclas/input_cox_trend.rds")
```
setting refernce levels for categorical data for better readability
```{r}
input_cox_trend$'smoking_status' <- relevel(input_cox_trend$'smoking_status', ref = "No smoker")
input_cox_trend$'alcohol_risk_consumption' <- relevel(input_cox_trend$'alcohol_risk_consumption', ref = "No risk consumption")
input_cox_trend$'cat_eGFR' <- relevel(input_cox_trend$'cat_eGFR', ref = "normal value")
```
# Create and run Cox model benchmark
Create survival object
```{r}
surv_object <- Surv(input_cox_trend$time_to_event, input_cox_trend$event)
```
Fit Cox Proportional Hazards model
```{r}
cox_model <- coxph(surv_object ~ .-time_to_event -event, data = input_cox_trend)
summary(cox_model)  
```
...
```{r}
cox_model <- coxph(formula = surv_object ~ delta_last + val_HbA1c + sexe + 
    age + smoking_status + qmedea + hypertension + dyslipidemia + 
    ischemic_heart_disease + chronic_kidney_disease + BMI + diastolic_blood_pressure + 
    triglycerides + ACR + cat_eGFR, data = input_cox_trend)
summary(cox_model)
```
Correlation plot
...
cloglog_plot(cox_model, input_cox_trend, "sexe")
cloglog_plot(cox_model, input_cox_trend, "smoking_status")
cloglog_plot(cox_model, input_cox_trend, "alcohol_risk_consumption")
cloglog_plot(cox_model, input_cox_trend, "qmedea")
cloglog_plot(cox_model, input_cox_trend, "hypertension")
cloglog_plot(cox_model, input_cox_trend, "dyslipidemia")
cloglog_plot(cox_model, input_cox_trend, "peripheral_artey_disease")
cloglog_plot(cox_model, input_cox_trend, "chronic_kidney_disease")
```
...
```{r}
check_log_linearity(cox_model, input_cox_trend, "val_HbA1c")
check_log_linearity(cox_model, input_cox_trend, "age")
check_log_linearity(cox_model, input_cox_trend, "BMI")
check_log_linearity(cox_model, input_cox_trend, "diastolic_blood_pressure")
check_log_linearity(cox_model, input_cox_trend, "triglycerides")
check_log_linearity(cox_model, input_cox_trend, "ACR")
check_log_linearity(cox_model, input_cox_trend, "delta_last")
```
Creating one single output
```{r}
covariates <- c("val_HbA1c", "age", "BMI",
                "diastolic_blood_pressure",
                "triglycerides", "ACR", "delta_last")

# Save to optimized PDF
pdf("final_plots/log-linearity_violations_trend.pdf", width = 10, height = 8)
par(mfrow = c(4, 2), mar = c(4, 2, 2, 1))

for (var in covariates) {
  check_log_linearity(cox_model, input_cox_trend, var)
}
...
cox_frailty <- coxph(surv_object ~ val_HbA1c + sexe + age + smoking_status + 
    ...
    ACR + frailty(qmedea), data = input_cox_trend)
```
...
```{r}
cox_model_upd <- coxph(formula = surv_object ~ ns(delta_last, df=8) + ns(val_HbA1c, df=8) + sexe + 
    ns(age, df=8) + smoking_status + qmedea + hypertension + dyslipidemia + 
    ischemic_heart_disease + chronic_kidney_disease + ns(BMI, df=8) + ns(diastolic_blood_pressure, df=8) + 
    ns(triglycerides, df=8) + ns(ACR, df=8) + cat_eGFR, data = input_cox_trend)
...
# Save to optimized PDF
pdf("final_plots/log-linearity_violations_fixed_trend.pdf", width = 10, height = 8)
par(mfrow = c(4, 2), mar = c(4, 2, 2, 1))

for (var in covariates) {
  check_log_linearity(cox_model_upd, input_cox_trend, var)
}
...
```{r}
check_log_linearity(cox_model_upd, input_cox_trend, "val_HbA1c")
check_log_linearity(cox_model_upd, input_cox_trend, "age")
check_log_linearity(cox_model_upd, input_cox_trend, "BMI")
check_log_linearity(cox_model_upd, input_cox_trend, "systolic_blood_pressure")
check_log_linearity(cox_model_upd, input_cox_trend, "diastolic_blood_pressure")
check_log_linearity(cox_model_upd, input_cox_trend, "cholesterol_LDL")
check_log_linearity(cox_model_upd, input_cox_trend, "triglycerides")
check_log_linearity(cox_model_upd, input_cox_trend, "ACR")
```
...
cox_model_ns_tt <- coxph(surv_object ~ sexe + ns(age, df=8) + tt(age) + `smoking status` + `alcohol risk consumption` + 
    ...
    data = input_cox_trend, tt=function(x,t,...) x * log(t) )
...
cox_model_tt_test <- coxph(surv_object ~ sexe + ns(age, df=8) + `smoking status` + `alcohol risk consumption` + 
    ...
    data = input_cox_trend, tt=function(x,t,...) x * t)
...
```{r}
input_cox_trend$time_to_event <- input_cox_trend$time_to_event + 0.0001
```
Splitting data by time-inertvals
```{r}
input_split <- survSplit(Surv(time_to_event, event) ~.,
                  data = input_cox_trend,
                  cut = c(1600, 3200),
                  episode = "interval")
```
...
ns_delta_last <- ns(input_split$delta_last, df = 8)
input_split$ns_delta_last1 <- ns_delta_last[,1]
input_split$ns_delta_last2 <- ns_delta_last[,2]
input_split$ns_delta_last3 <- ns_delta_last[,3]
input_split$ns_delta_last4 <- ns_delta_last[,4]
input_split$ns_delta_last5 <- ns_delta_last[,5]
input_split$ns_delta_last6 <- ns_delta_last[,6]
input_split$ns_delta_last7 <- ns_delta_last[,7]
input_split$ns_delta_last8 <- ns_delta_last[,8]
```
Actual model call
```{r}
cox_model_comb <- coxph(Surv(tstart, time_to_event, event) ~ 
  ...
  (ns_delta_last1 + ns_delta_last2 + ns_delta_last3 + ns_delta_last4 + ns_delta_last5 + ns_delta_last6 + ns_delta_last7 + ns_delta_last8) * factor(interval) +
...
```
plot(cox.zph(cox_model_comb)) --> DO NOT COMPILE --> too many useless plots

Test for linearity of combined updated model 
```{r}
plot(predict(cox_model_comb), residuals(cox_model_comb, type="martingale"),
      xlab="fitted values", ylab="Martingale residuals", main="Residual plot", las=1)
      abline(h = 0, col = "red")
      lines(smooth.spline(predict(cox_model_comb),
      residuals(cox_model_comb, type="martingale")), col="blue")
```
Reverse-engineering for simplest call
```{r}
library(Hmisc)

surv_object <- Surv(time = input_cox_trend$time_to_event, event = input_cox_trend$event)
# Variables ranked by absolute z-value from output of simple call earlier
var_ranked <- c("val_HbA1c", "delta_last", "age", "sexe", "smoking_status", 
                "qmedea", "BMI", "diastolic_blood_pressure" , "hypertension", 
                "dyslipidemia", "ischemic_heart_disease",
                "chronic_kidney_disease", "cat_eGFR", "triglycerides", "ACR")
...
for (k in step_sizes) {
  ...
  model_k <- coxph(formula_k, data = input_cox_trend)
  pred_k <- predict(model_k, type = "lp")
  ci_k <- 1 - rcorr.cens(pred_k, Surv(input_cox_trend$time_to_event, input_cox_trend$event))[1]
...
print(p)
ggsave(paste0("final_plots/influence_number_of_variables_trend.pdf"), width = 6, height = 4, units = "in")
```
...
Calling of function
```{r}
plot_separation_km(input_cox_trend, factor_col = "smoking_status", output_path= "final_plots/input_cox_trend_separation/separation_smoking_status_trend.pdf")
plot_separation_km(input_cox_trend, factor_col = "alcohol_risk_consumption", output_path= "final_plots/input_cox_trend_separation/separation_alcohol_risk_cosumption_trend.pdf")
plot_separation_km(input_cox_trend, factor_col = "sexe", output_path= "final_plots/input_cox_trend_separation/separation_sexe_trend.pdf")
plot_separation_km(input_cox_trend, factor_col = "qmedea", output_path= "final_plots/input_cox_trend_separation/separation_qmedea_trend.pdf")
plot_separation_km(input_cox_trend, factor_col = "cat_eGFR", output_path= "final_plots/input_cox_trend_separation/separation_cat_eGFR_trend.pdf")
```
